...
import pandas as pd
import numpy as np
from STResnet_dataset import *
import matplotlib.pyplot as plt
from Metric import *
from sklearn.model_selection import KFold
#datatype set to double
torch.set_default_tensor_type(torch.DoubleTensor)
path = "NN/STDNN/STDNN_normal"
#train device
device = torch.device("cuda")

def sample_train(epoch_num,learningrate,batchsize,ev_num,day_num,month_num,year_num,random_state,device):
    #load data
...
def station_train(epoch_num,learningrate,batchsize,ev_num,day_num,month_num,year_num,random_state,device):
    # load data
    inputpath = "pm+gwr_day+time+ev.csv"
    df = pd.read_csv(inputpath)
    outputpath="/home/suheng/文档/深度学习/实验结果/STResnet_station/"
    # 划分10折训练集和测试集
    ...
    final_station_MAE={}
    n = 0
    kf = KFold(n_splits=10, shuffle=True, random_state=random_state)  # station cv
            ...
            with torch.no_grad():
                ...
                if i >= 800:
                    ...
                    if temp_r2 >= max_test_r2:
                        ...
                        torch.save(STDNN, path + "STDNN" + str(n) + "_station.pt")
    ...
    df.to_csv(outputpath+"/WDEM/Station_random_state{}.csv".format(random_state),index_label='code')
    print(station_list_10fold)

if __name__ == '__main__':
    random_state=345
    print(random_state)
    #sample_train(1000,0.005,640,5,15,6,2,random_state=random_state,device=device)
    station_train(1000,0.005,640,5,15,6,2,random_state=random_state,device=device)
