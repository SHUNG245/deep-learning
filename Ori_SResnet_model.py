...
class Ori_Resnet(nn.Module):
    def __init__(self,train_x_size):
        super().__init__()
        self.relu=nn.ReLU()
        self.bn0 = nn.BatchNorm1d(train_x_size+2+3)
        self.hd_layer1 = nn.Linear(train_x_size+2+3, 64)
        ...
        self.hd_layer6 = nn.Linear(64,train_x_size+2+3)
        self.bn6 = nn.BatchNorm1d(train_x_size+2+3)
        self.hd_layer7 = nn.Linear(train_x_size+2+3,1)

    def tensor_merge(self,x,ev,time):
        x = torch.cat([x, ev,time], 1)
        return x

    def forward(self,x,ev,time):
        x=self.tensor_merge(x,ev,time)
        input= self.bn0(x)
