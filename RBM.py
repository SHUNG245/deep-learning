...
class RBM(nn.Module):
    ...
    def __init__(self,visible_units=9,hidden_units=64,k=1,batch_size=16,
    ...
    def reset_parameters(self):
        ...
        )#初始化权重参数,使其均匀分布，gain缩放因子，fan_in:visible_units,fan_out:hidden_units，
         # nn.init.calculate_gain：主要功能：计算激活函数的方差变化尺度，self.activation_name：激活函数名称
    ...
    def to_hidden(self,vis_prob):
        ...
        Args:
            vis_prob: Visible layer probability. It's also RBM input. Size is
                (n_samples(输入样本数) , n_features（显层神经元个数 ）).
        ...
        hid_sample = torch.bernoulli(hid_prob)#从伯努利分布中提取二进制随机数（0或1）
    ...
    def contrastive_divergence(self, input_data, training =True,
                               n_gibbs_sampling_steps=1, lr=0.01):
        """
        ...
        """
        # Positive phase 隐层神经元被激活情况
        positive_hid_prob, positive_hid_dis = self.to_hidden(input_data)

        # Calculate energy via positive side 把每个神经元的所有样本能量加起来
        positive_associations = torch.matmul(input_data.t(),positive_hid_dis)

        # Negative phase
        hidden_activitons = positive_hid_dis
        vis_prob = torch.rand(input_data.size(),device= self.device)#初始化显层激活概率
        hid_prob = torch.rand(positive_hid_prob.size(), device=self.device)#初始化隐层激活概率
        for i in range(n_gibbs_sampling_steps):
            vis_prob, _ =self.to_visible(hidden_activitons)
            hid_prob,hidden_activitons = self.to_hidden(vis_prob)
    ...
    def train_rbm(self,train_dataloader,epoch=50):
        ...
        for epoch_i in range(1,epoch+1):
            n_batches = int(len(train_dataloader))#返回的是batch个数

            cost_ = torch.DoubleTensor(n_batches,1)
            grad_ = torch.DoubleTensor(n_batches,1)

            #Train_loader contains input and output data. However, training
            # of RBM doesn't require output data
            for i , batch in enumerate(train_loader):
                cost_[i-1],grad_[i-1] = self.contrastive_divergence(batch[0],True,self.k,lr = self.learning_rate)
            if epoch_i %10 ==0:
